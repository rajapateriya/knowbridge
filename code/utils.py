...
from paths import DATA_DIR, KNOWLEDGE_FPATH, ENV_FPATH


def load_knowledge(knowledge_external_id="yzN0OCQT7hUS"):
    """Loads the knowledge markdown file.

    Returns:
        Content of the knowledge as a string.

    Raises:
        FileNotFoundError: If the file does not exist.
        IOError: If there's an error reading the file.
    """
    knowledge_fpath = Path(os.path.join(DATA_DIR, f"{knowledge_external_id}.md"))

    # Check if file exists
    if not knowledge_fpath.exists():
        raise FileNotFoundError(f"knowledge file not found: {knowledge_fpath}")

    # Read and return the file content
    try:
        with open(knowledge_fpath, "r", encoding="utf-8") as file:
            return file.read()
    except IOError as e:
        raise IOError(f"Error reading knowledge file: {e}") from e


def load_all_knowledge_base(knowledge_dir: str = DATA_DIR) -> list[tuple[str, str]]:
    """Loads all the knowledge markdown files in the given directory.

    Returns:
        List of (content, source_name) tuples.
    """
    knowledges = []
    for pub_id in sorted(os.listdir(knowledge_dir)):
        if pub_id.endswith(".md"):
            source_name = pub_id.replace(".md", "")
            knowledges.append((load_knowledge(source_name), source_name))
    return knowledges
